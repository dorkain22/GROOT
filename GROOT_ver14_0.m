%General notes: This version (14.0) contains:
...
lblPostHoc           = uilabel(uiLeft,'Position',[10 200 120 20], ...
                                'Text','Post-hoc test:');
ddPostHoc            = uidropdown(uiLeft,'Position',[130 200 170 22], ...
...
% ─────────────────────────  RUN ANOVA  ───────────────────────────────
    function runAnova(~,~)
        ...
        for g = 1:numel(selGroups)
            ...
            groupVectors{end+1}    = mean(mat,2,'omitnan');  %#ok<AGROW>
        ...
        title (axAnova, sprintf('One-way ANOVA  p = %.3g', p));
        grid  (axAnova,'on');
        hold  (axAnova,'off');
